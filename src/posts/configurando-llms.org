...
e era chegada a hora de eu praticar  o que acredito quando o assunto fosse LLMs,
para não me  tornar apenas mais um programador velho  e mal-humorado achando que
tinha que fazer tudo em C e Lisp (apesar de que sou exatamente assim).
...
modelos de linguagem localmente usando Ollama, em vez de gastar rios de dinheiro
com as soluções pagas do mercado. Eis  aqui os passos que segui e algumas coisas
mais interessantes.
...
geral de Python e/ou Django e afins.
...
não  é muito  meu forte.  Mas recentemente  adquiri um  "laptop gamer",  um Dell
...
Outra coisa legal de se ter é, também, uma interface gráfica de alguma forma. Eu
acabei optando por usar o Open WebUI,  que aparentemente é extensível a ponto de
tolerar mais  ferramentas caso você  queira partir  para coisas como  geração de
imagens,  por exemplo.  Mas como  não é  meu objetivo,  realizei um  deploy mais
simples.
...
gráfica, mas depois de reclamar um pouco, eu só aceitei.
...
já com o Ollama embutido, caso você queira algo mais unificado e não vá depender
do Ollama para outros serviços.

Esse ponto não  é um grande problema  caso você esteja usando  Docker Desktop no
Windows[fn:5], porque  ele já exporta  um /host/  customizado para que  você tenha
acesso às portas do seu computador.
...
pull~ na  imagem novamente. Então  acabei montando um ~docker-compose.yml~  para me
ajudar com isso:
...
no Docker Compose, pode remover isso. Vai dar bom.
...
padrão da  aplicação para o  computador. Em vez  disso, eu estou  configurando a
aplicação diretamente (através da variável de ambiente ~PORT~) para rodar na porta
~3111~. Esse detalhe do modo de  networking do contêiner pode ser importante, caso
você precise alterar esse arquivo do Compose.
...
Tudo agora funcionou  muito bem, mas estamos  em um ponto crítico:  Tudo isso só
funciona localmente.  E se  você for  como eu, muito  provavelmente você  tem um
/firewall/  bem-configurado  no  seu  computador que  não  esteja  permitindo,  no
momento, acesso a nenhuma porta sua. Correto? Correto.

No meu  caso, eu instalei  o Uncomplicated Firewall (UFW).  Ele é uma  camada em
cima  do ~iptables~  que garante  certa simplicidade.  Depois de  instalar o  ~ufw~,
ative-o através do ~systemd~.
...
computador.  O primeiro  é que  eu  precisava acessar  o Open  WebUI pelo  menos
no  meu celular;  o segundo,  é que  eu precisava  acessar o  Ollama diretamente
através do meu laptop de trabalho,  um sistema com Ubuntu, que eu potencialmente
levaria para fora da minha casa.

Esse  é o  caso para  alguma aplicação  que permita  o tunelamento  entre vários
dispositivos.  Fortunadamente,  existe  uma  solução muito  legal  pra  isso:  o
...
segura entre os  três dispositivos. Então, a princípio, bastou  seguir os passos
de instalação para Android e para os dois Linux, e fazer login na rede.

Uma vez na rede,  do ponto de vista do meu computador,  pude executar o seguinte
comando e obter saídas como essas:
...
comando, eu poderia inclusive criar um script para facilitar esse tipo de coisa.
...
não necessariamente  precise colocar os  argumentos entre parênteses (a  não ser
...
a usar esse processo para trabalhar,  mas certamente já tenho algumas opiniões a
respeito de como tudo isso funciona.
...
são excelente ferramentas de apoio, mas não  são balas de prata, e certamente me
...
* Footnotes
[fn:11] [[https://github.com/charmbracelet/glow][Glow]] é  uma ferramenta de renderização de Markdown  para terminal. Estou
usando para que a saída fique "bonitinha", mesmo.

[fn:10] Aqui eu  usei a opção ~status  numbered~ para que você veja  que as regras
sempre estão numeradas  na ordem em que  se apresentam. Se por  acaso você tiver
regras a mais  ou a menos que queira  remover, é só uma questão de  usar um ~sudo
ufw delete <número>~.

[fn:9] Veja que,  em parte, isso abre o pressuposto  para eu conseguir inclusive
instalar o ~ollama~ no  Ubuntu, sem rodar o serviço em si, e  usar uma variável de
ambiente para se  conectar ao meu computador  e gerenciar modelos, mas  não é um
grande problema  porque só eu  tenho acesso a  esse computador, e  se porventura
perder o acesso, posso "chutá-lo" da minha rede pessoal.
...
do WSL  e instalar  Docker dentro  dele, é  um pouco  melhor, mas  você continua
...
elas. Então não me leve a mal, caso  você ainda não sabe o que é... corra atrás,
